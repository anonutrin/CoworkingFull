...
const serverHost = 'https://allrenda.ru';
...
async function fetchLocations(sphere, price) {
    ...
    const url = `${serverHost}:${serverPort}/locations?${uriComponent}`;
...
$map.addEventListener('click', async e => {
    if (e.target.classList.contains('choose-btn')) {
        prompt('', JSON.stringify(tg.initDataUnsafe));
...
async function fetchShowLocation(chat_id, location_id) {
    ...
    const url = `${serverHost}:${serverPort}/show?${data.toString()}`;
...
async function fetchUserInfo(chat_id) {
    ...
    const url = `${serverHost}:${serverPort}/user-info?${data.toString()}`;
...
async function main() {
    ...
    const coordinates = user_info?.latitude && user_info?.longitude ? [user_info.latitude, user_info.longitude] : getDefaultCoordinates();
